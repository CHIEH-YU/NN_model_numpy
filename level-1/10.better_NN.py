...
class xW_b:
    ...
    def backprop(self, dL):
        ...
        self.db = db*(1./self.batch)
        self.dW = dW*(1./self.batch)
        return dL_prev
    def get_dWb(self):
        return self.dW, self.db
    def update(self, update_dW, update_db):
        self.W = self.W - update_dW
        self.b = self.b - update_db
...
class Tanh():
    def forward(self, inp_layer):
        temp = (np.exp(inp_layer*2))
        self.tanh = (temp-1)/(temp+1)
        return self.tanh
    def backprop(self, dL):
        return dL*(1-((self.tanh)**2)) 
...
class SGD_optimizer:
    def __init__(self, lr):
        self.lr = lr
    def __call__(self, dWbs):
        output = []
        for _ in dWbs:
            output.append(_*self.lr)
        return output

class momentum_optimizer:
    def __init__(self, lr, fraction=0.9):
        self.lr = lr
    ...
    def __call__(self, dWbs):
        ...
        if self.step == 0:
            ...
            for idx,_ in enumerate(dWbs):
                this_step_update = _*self.lr
                prev_step_update = self.fraction * self.prev_output[idx]
                sum_up_update = this_step_update + prev_step_update
                output.append(sum_up_update)
...
dense2  = xW_b((30,10))
tanh = Tanh()
SGD = momentum_optimizer(lr=0.001)
for epoch in range(20):
    for _ in range(5000):
        ...
        #back propagation
        dL = dense2.backprop(dL)
        dL = tanh.backprop(dL)
        dL = dense1.backprop(dL)

        dW2, db2 = dense2.get_dWb()
        dW1, db1 = dense1.get_dWb()

        dW2, db2, dW1, db1 = SGD([dW2,db2,dW1,db1])
        dense2.update(dW2,db2)
        dense1.update(dW1,db1)
